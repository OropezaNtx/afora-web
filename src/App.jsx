    ...
    <div className="phone-wrap" aria-label="Representación de la aplicación móvil ALURA">
      <div className="phone">
        ...
        <div className="phone-screen">
          ...
          <div className="phone-product-visual" style={{ backgroundImage: `url(${ASSETS.fieldApp})` }}>
            <RouteCanvas compact />
          </div>
          <div className="phone-status"><i /> Operación en curso</div>
          <div className="event-block"><span>REGISTRO</span><strong>Evento de campo</strong><small>19:32:14 · GPS 5.2 m</small></div>
          <div className="phone-actions"><button>−</button><strong>07</strong><button>+</button></div>
...
function OperationsMockup() {
  return (
    <div className="ops-window product-reveal">
      <div className="ops-product-image" style={{ backgroundImage: `url(${ASSETS.operations})` }} />
      <div className="ops-bar"><span /><span /><span /><small>Centro de Operaciones ALURA</small></div>
      <div className="ops-layout">
        <aside><b>ALURA</b><span className="active">Overview</span><span>Operaciones</span><span>Equipos en vivo</span><span>Integridad</span><span>Calidad</span></aside>
        <div className="ops-main">
          <div className="ops-heading"><div><small>OPERACIÓN</small><strong>Visibilidad operacional</strong></div><span className="live-pill"><i /> LIVE</span></div>
...
function FragmentationVisual() {
  const items = [
    ...
    ['EVIDENCIA', 'frag-4'], ['MENSAJERÍA', 'frag-5'], ['REPORTES', 'frag-6'],
  ];
  return (
    <div className="fragmentation reveal">
      <svg className="fragment-lines" viewBox="0 0 1000 440" preserveAspectRatio="none" aria-hidden="true">
        <path d="M115 110 C290 150 345 205 500 220" />
        <path d="M865 84 C720 125 650 180 500 220" />
        <path d="M160 350 C300 320 365 260 500 220" />
        <path d="M860 330 C720 300 660 252 500 220" />
        <path d="M455 55 C470 115 485 160 500 220" />
        <path d="M595 395 C560 330 530 278 500 220" />
      </svg>
      {items.map(([label, cls]) => <span className={cls} key={label}>{label}</span>)}
      <div className="fragment-core"><span>ALURA</span><small>contexto conectado</small></div>
    </div>
  );
}
...
function EvidenceVisual() {
  return (
    <div className="evidence-frame">
      <div className="evidence-photo" style={{ backgroundImage: `linear-gradient(180deg, rgba(5,6,8,.08), rgba(5,6,8,.72)), url(${ASSETS.evidence})` }}>
        <div className="evidence-placeholder"><MapPin size={25} /><span>Evidencia asociada</span></div>
      </div>
      <div className="evidence-data"><small>EVENTO 048</small><strong>Registro validado</strong><span>19:32:14</span><span>19.4326, -99.1332</span><span>Precisión GPS · 5.2 m</span><div className="evidence-status"><i /> Ligado a la operación</div></div>
    </div>
  );
}
...
function HealthPanel() {
  const rows = [
    ['GPS', 'HEALTHY'], ['TRACK', 'COMPLETE'], ['SYNC', 'VERIFIED'], ['DATA', 'COMPLETE'],
  ];
  return (
    <div className="health-panel reveal">
      ...
      <small>La interfaz usa estados derivados de la operación; los valores reales dependen de cada levantamiento.</small>
    </div>
  );
}
    ...
    <div id="top">
      ...
      <main>
        <section className="hero">
          ...
          <div className="container hero-content"><span className="eyebrow hero-eyebrow"><i /> Plataforma de operaciones de campo</span><h1>El campo está<br /><em>en movimiento.</em></h1><p>ALURA conecta personas, recorridos, registros y supervisión dentro de una misma operación.</p><div className="hero-actions"><a className="button" href="#demo">Solicitar demo <ArrowRight size={17} /></a><a className="text-link" href="#platform">Descubrir la plataforma <ArrowDown size={16} /></a></div></div>
          <div className="hero-caption"><span>CAMPO</span><i /><span>INFORMACIÓN</span></div>
        </section>

        <section className="statement section"><div className="container narrow reveal"><span className="eyebrow">Una operación, no herramientas aisladas</span><h2>La información de campo no debería reconstruirse después.</h2><p>Cuando recorridos, archivos, ubicación, mensajes y registros viven separados, entender qué ocurrió se convierte en otro trabajo. ALURA conserva el contexto desde el momento en que se genera.</p></div><FragmentationVisual /></section>
        ...
        <Scene eyebrow="01 · Movimiento" title={<>Cada recorrido deja <em>información.</em></>} copy="ALURA registra el contexto geográfico de la operación: recorrido, tiempo, eventos y calidad de ubicación forman parte de la misma historia." visual={<div className="route-showcase"><RouteCanvas /><div className="route-readout"><Route size={18} /><span>TRACK ACTIVO</span><b>18.42 km</b></div></div>} />

        <Scene eyebrow="02 · Captura" title={<>Más que un dato.<br /><em>Contexto.</em></>} copy="La aplicación Android estructura la actividad alrededor de operaciones reales. Cada registro conserva cuándo ocurrió, dónde ocurrió y a qué operación pertenece." visual={<PhoneMockup />} reverse />

        <Scene eyebrow="03 · Evidencia" title={<>Lo que ocurre en campo, <em>queda ligado</em> a la operación.</>} copy="Registros, ubicación, estados y evidencia pueden convivir dentro del mismo flujo para facilitar revisión y trazabilidad posterior." visual={<EvidenceVisual />} />
        ...
        <section id="operations" className="operations section"><div className="container"><div className="operations-intro reveal"><span className="eyebrow">05 · Supervisión</span><h2>Del campo al<br /><em>centro de operaciones.</em></h2><p>La actividad deja de estar encerrada en el dispositivo. ALURA la convierte en visibilidad operacional.</p></div><div className="operations-stage reveal"><OperationsMockup /></div></div></section>

        <section id="integrity" className="integrity section"><div className="container integrity-grid"><div className="integrity-copy reveal"><span className="eyebrow">06 · Integridad y calidad</span><h2>Capturar no es suficiente.<br /><em>Hay que poder confiar.</em></h2><p>ALURA analiza el estado operativo de los levantamientos para ayudar a distinguir información completa de operaciones que necesitan atención.</p></div><HealthPanel /></div></section>

        <section id="use-cases" className="ecosystem section"><div className="container"><div className="section-heading reveal"><span className="eyebrow">Una plataforma, diferentes operaciones</span><h2>Probado en movilidad.<br /><em>Diseñado para el campo.</em></h2><p>El núcleo de ALURA permite estructurar distintos flujos donde ubicación, tiempo, evidencia y trazabilidad importan.</p></div><div className="ecosystem-stage reveal" style={{ backgroundImage: `linear-gradient(rgba(8,10,14,.88), rgba(8,10,14,.94)), url(${ASSETS.mapTexture})` }}><div className="eco-ring ring-one" /><div className="eco-ring ring-two" /><div className="eco-core"><BrandMark /><small>FIELD OPERATIONS CORE</small></div><span className="eco-node n1">Movilidad</span><span className="eco-node n2">Levantamientos</span><span className="eco-node n3">Inspecciones</span><span className="eco-node n4">Inventarios</span><span className="eco-node n5">Auditorías</span></div></div></section>

        <section id="demo" className="cta section"><RouteCanvas compact subtle /><div className="container cta-content reveal"><span className="eyebrow">ALURA</span><h2>Conoce la operación<br />desde otra perspectiva.</h2><p>Explora cómo ALURA puede conectar el trabajo de campo con supervisión, integridad e información.</p><a className="button" href="mailto:contacto@alura.mx">Solicitar una demo <ArrowRight size={17} /></a></div></section>
      </main>

      <footer><div className="container footer-grid"><div><BrandMark /><p>Field operations, connected.</p></div><div><strong>Plataforma</strong><a href="#platform">Cómo funciona</a><a href="#operations">Operaciones</a><a href="#integrity">Integridad</a></div><div><strong>Producto</strong><a href="#use-cases">Casos de uso</a><a href="#demo">Solicitar demo</a></div><div className="footer-status"><span><i /> Producto en desarrollo activo</span><small>© 2026 ALURA</small></div></div></footer>
    </div>
  );
}
